...
To install the chart with the release name `choerodon-platform`:

```console
$ helm repo add c7n https://openchart.choerodon.com.cn/choerodon/c7n
$ helm repo update
$ helm install choerodon-platform c7n/choerodon-platform
```
...
| containerPort.actuatorPort | int | `8101` | server management port |
| containerPort.serverPort | int | `8100` | server port |
...
| extraEnv.HZERO_LOV_SQL_ENABLED | bool | `true` |  |
| extraEnv.HZERO_LOV_VALUE_ENABLED | bool | `true` |  |
| extraEnv.HZERO_OAUTH_PASSWORD_PRIVATE_KEY | string | `""` |  |
| extraEnv.HZERO_OAUTH_PASSWORD_PUBLIC_KEY | string | `"MFwwDQYJKoZIhvcNAQEBBQADSwAwSAJBAJL0JkqsUoK6kt3JyogsgqNp9VDGDp+t3ZAGMbVoMPdHNT2nfiIVh9ZMNHF7g2XiAa8O8AQWyh2PjMR0NiUSVQMCAwEAAQ=="` |  |
| extraEnv.HZERO_PLATFORM_HTTP_PROTOCOL | string | `"http"` |  |
| extraEnv.HZERO_PLATFORM_INIT_CACHE | bool | `true` |  |
| extraEnv.HZERO_REGIST_ENTITY_ENABLE | bool | `false` |  |
...
| image.repository | string | `"c7n/choerodon-platform"` | service image repository |
...
| service.nodePort | object | `{"actuator":30107,"server":30106}` | Specify the nodePort value for the LoadBalancer and NodePort service types. |
| service.port | object | `{"actuator":8101,"server":8100}` | server Service port |

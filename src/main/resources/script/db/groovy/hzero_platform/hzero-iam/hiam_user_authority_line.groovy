...
databaseChangeLog(logicalFilePath: 'script/db/hiam_user_authority_line.groovy') {
    ...
    changeSet(author: "Admin@hand-china.com", id: "hiam_user_authority_line-2021-05-18-version-2") {
        addColumn (tableName: "hiam_user_authority_line") {
    ...
    changeSet(author: "Admin@hand-china.com", id: "hiam_user_authority_line-2021-06-22-version-3") {
        dropNotNullConstraint (tableName: "hiam_user_authority_line", columnName: "data_source", columnDataType: "varchar(" + 30* weight_c + ")")
        //mysql在删除非空约束时候会清空默认值
        if (helper.isMysql()) {
            addDefaultValue (tableName: "hiam_user_authority_line", columnName: "data_source", columnDataType: "varchar(" + 30* weight_c + ")", defaultValue: "DEFAULT")
        }
    }
    changeSet(author: "Admin@hand-china.com", id: "hiam_user_authority_line-2021-08-16-version-4") {
        dropUniqueConstraint(tableName: 'hiam_user_authority_line', constraintName: 'hiam_user_authority_line_u1')

        if (!helper.isMysql()) {
            setColumnRemarks (tableName: "hiam_user_authority_line", columnName: "data_id", remarks: "数据标识")
        }
        if (helper.isMysql()) {
            renameColumn (tableName: "hiam_user_authority_line", oldColumnName: "data_id", newColumnName: "data_id", columnDataType: "varchar(" + 120* weight_c + ")", remarks: "数据标识")
            //mysql在修改列名时候会清空非空约束和默认值
            if (helper.isMysql()) {
                addNotNullConstraint (tableName: "hiam_user_authority_line", columnName: "data_id", columnDataType: "varchar(" + 120* weight_c + ")")
            }
        }
        modifyDataType (tableName: "hiam_user_authority_line", columnName: "data_id", newDataType: "varchar(" + 120* weight_c + ")")

...
public class LanguageAutoUpdateTask implements Runnable {
    ...
    private final MybatisConfigurationProperties properties;
    ...
    public LanguageAutoUpdateTask(RedisHelper redisHelper,
                                  MybatisConfigurationProperties properties,
    ...
    public MybatisConfigurationProperties getProperties() {
        return properties;
    }
    ...
    private boolean getLock() {
        ...
        if (expire == -1L) {
            long expireSeconds = properties.getLanguage().getAutoRefresh().getInterval().getSeconds() << 1;
    ...
    private void updateLanguages(Map<String, String> languages) {
        SafeRedisHelper.execute(HZeroService.Platform.REDIS_DB, redisHelper, () -> redisHelper.delKey(LANGUAGE));
